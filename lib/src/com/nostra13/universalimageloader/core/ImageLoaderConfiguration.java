 ...
 * @since 1.0.0<br>
 *        图片缓存的全局配置，主要有线程类、缓存大小、磁盘大小、图片下载与解析、日志方面的配置。<br>
 *        E.g. <br>
 *        ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(context)<br>
 *        .memoryCacheExtraOptions(480, 800) // default = device screen dimensions<br>
 *        .diskCacheExtraOptions(480, 800, null)<br>
 *        .taskExecutor(...)<br>
 *        .taskExecutorForCachedImages(...)<br>
 *        .threadPoolSize(3) // default<br>
 *        .threadPriority(Thread.NORM_PRIORITY - 2) // default<br>
 *        .tasksProcessingOrder(QueueProcessingType.FIFO) // default<br>
 *        .denyCacheImageMultipleSizesInMemory()<br>
 *        .memoryCache(new LruMemoryCache(2 * 1024 * 1024))<br>
 *        .memoryCacheSize(2 * 1024 * 1024)<br>
 *        .memoryCacheSizePercentage(13) // default<br>
 *        .diskCache(new UnlimitedDiscCache(cacheDir)) // default<br>
 *        .diskCacheSize(50 * 1024 * 1024)<br>
 *        .diskCacheFileCount(100)<br>
 *        .diskCacheFileNameGenerator(new HashCodeFileNameGenerator()) // default<br>
 *        .imageDownloader(new BaseImageDownloader(context)) // default<br>
 *        .imageDecoder(new BaseImageDecoder()) // default<br>
 *        .defaultDisplayImageOptions(DisplayImageOptions.createSimple()) // default<br>
 *        .writeDebugLogs()<br>
 *        .build();<br>
 */
public final class ImageLoaderConfiguration {
    ...
    // 图片解码器，内部可使用常用的BitmapFactory.decode(…)将图片资源解码成Bitmap对象。
    ...
    /**
     * Builder for {@link ImageLoaderConfiguration} 建造者模式,用于构造参数繁多的ImageLoaderConfiguration。
     * 
     * @author Sergey Tarasevich (nostra13[at]gmail[dot]com)
     */
    public static class Builder {
        ...
        /**
         * Sets options for memory cache 保存的每个缓存文件的最大长宽
         * 
         ...
         *            null-ok; {@linkplain BitmapProcessor Bitmap processor} which process images before saving them in
         *            disc cache
         */
         ...
         * <br />
         * If you set the same executor for {@linkplain #taskExecutor(Executor) general tasks} and tasks about cached
         * images (this method) then these tasks will be in the same thread pool. So short-lived tasks can wait a long
         * time for their turn.<br />
         * <br />
         * <b>NOTE:</b> If you set custom executor then following configuration options will not be considered for this
         * executor:
         * <ul>
         ...
         * <p>
         * 设置内存缓存不允许缓存一张图片的多个尺寸，默认允许。 后面会讲到 View 的 getWidth() 在初始化前后的不同值与这个设置的关系。
         */
         ...
         * <b>NOTE:</b> If you use this method then
         * {@link com.nostra13.universalimageloader.cache.disc.impl.ext.LruDiskCache LruDiskCache} will be used as disk
         * cache. You can use {@link #diskCache(DiskCache)} method for introduction your own implementation of
         * {@link DiskCache}
         * <p>
         * 设置磁盘缓存的最大字节数<br>
         * 如果大于 0 或者下面的maxFileCount大于 0，默认的DiskCache会用LruDiskCache，否则使用无大小限制的UnlimitedDiskCache。
         */
         ...
         * <p>
         * 设置磁盘缓存文件夹下最大文件数<br>
         * 如果大于 0 或者上面的maxCacheSize大于 0，默认的DiskCache会用LruDiskCache，否则使用无大小限制的UnlimitedDiskCache。
         */
        ...
        /**
         * diskCache默认值与diskCacheSize和diskCacheFileCount值有关；<br>
         * 如果他们有一个大于 0，则默认为LruDiskCache，否则使用无大小限制的UnlimitedDiskCache。<br>
         * <p>
         ...
         * DefaultConfigurationFactory.createImageDownloader()}<br>
         ...
         * DefaultConfigurationFactory.createImageDecoder()}<br>
        ...
        /**
         * Enables detail logging of {@link ImageLoader} work. To prevent detail logs don't call this method. Consider
         * {@link com.nostra13.universalimageloader.utils.L#disableLogging()} to disable ImageLoader logging completely
         * (even error logs)
         */
        ...
        /** 按照配置，生成 ImageLoaderConfiguration */
    ...
    /**
     * 装饰者模式. Prevents downloads from network (throws {@link IllegalStateException exception}).<br />
     * In most cases this downloader shouldn't be used directly.
     * 
     * @author Sergey Tarasevich (nostra13[at]gmail[dot]com)
     * @since 1.8.0
     *        <p>
     *        不允许访问网络的图片下载器，实现了ImageDownloader接口。包装一个ImageDownloader对象，通过在 getStream(…) 函数中禁止 Http 和 Https Scheme 禁止网络访问
     */
    ...
    /**
     * Decorator. Handles <a href="http://code.google.com/p/android/issues/detail?id=6066">this problem</a> on slow
     * networks using {@link com.nostra13.universalimageloader.core.assist.FlushedInputStream}.
     * 
     * @author Sergey Tarasevich (nostra13[at]gmail[dot]com)
     * @since 1.8.1
     *        <p>
     *        慢网络情况下的图片下载器，实现了ImageDownloader接口。 通过包装一个ImageDownloader对象实现，在 getStream(…) 函数中当 Scheme 为 Http 和 Https
     *        时，用FlushedInputStream代替InputStream处理慢网络情况.
     */

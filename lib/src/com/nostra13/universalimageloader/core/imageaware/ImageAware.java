 ...
 * through {@link com.nostra13.universalimageloader.core.ImageLoader ImageLoader}. It can wrap any Android
 * {@link android.view.View View} which can be accessed by {@link #getWrappedView()}. Wrapped view is returned in
 * {@link com.nostra13.universalimageloader.core.listener.ImageLoadingListener ImageLoadingListener}'s callbacks.
 * 
...
public interface ImageAware {
    ...
    /**
     * Returns width of image aware view. This value is used to define scale size for original image. Can return 0 if
     * width is undefined.<br />
     * Is called on UI thread if ImageLoader was called on UI thread. Otherwise - on background thread.
     */
    int getWidth();

    /**
     * Returns height of image aware view. This value is used to define scale size for original image. Can return 0 if
     * height is undefined.<br />
     * Is called on UI thread if ImageLoader was called on UI thread. Otherwise - on background thread.
     */
    int getHeight();
     ...
     * collected by GC.<br />
     * Is called on UI thread if ImageLoader was called on UI thread. Otherwise - on background thread.<br>
     ...
     * so ImageLoader won't process two image aware views with the same ID in one time. When ImageLoader get new task it
     * cancels old task with this ID (if any) and starts new task.
     * <p/>
     * It's reasonable to return hash code of wrapped view (if any) to prevent displaying non-actual images in view
     * because of view re-using.<br>
     * 得到唯一标识 id。ImageLoaderEngine中用这个 id 标识正在加载图片的ImageAware和图片内存缓存 key 的对应关系，图片请求前会将内存缓存 key 与新的内存缓存 key
     * 进行比较，如果不相等，则之前的图片请求会被取消。这样当ImageAware被复用时就不会因异步加载(前面任务未取消)而造成错乱了。
     */
    int getId();
     ...
     * Displays loaded and decoded image {@link android.graphics.Bitmap} in this image view aware. Actually it's used
     * only in {@link com.nostra13.universalimageloader.core.display.BitmapDisplayer BitmapDisplayer}.< br />
     * Is called on UI thread if ImageLoader was called on UI thread. Otherwise - on background thread.
     * 

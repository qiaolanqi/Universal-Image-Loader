...
public class ImageLoader {
     ...
     *             if <b>configuration</b> parameter is null<br>
     *             初始化配置参数
     */
     ...
     * @param imageAware
     *            {@linkplain com.nostra13.universalimageloader.core.imageaware.ImageAware Image aware view} which
     ...
     *            should display image
     * @param options
     *            {@linkplain com.nostra13.universalimageloader.core.DisplayImageOptions Options} for image decoding and
     *            displaying. If <b>null</b> - default display image options
     ...
     *            {@linkplain com.nostra13.universalimageloader.core.listener.ImageLoadingProgressListener Listener} for
     *            image loading progress. Listener fires events on UI thread if this method is called on UI thread.
     *            Caching on disk should be enabled in
     *            {@linkplain com.nostra13.universalimageloader.core.DisplayImageOptions options} to make this listener
     *            work.
     * @throws IllegalStateException
     *             if {@link #init(ImageLoaderConfiguration)} method wasn't called before
     * @throws IllegalArgumentException
    ...
    public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
        ...
        String memoryCacheKey = MemoryCacheUtils.generateKey(uri, targetSize);// [imageUri]_[width]x[height]
        engine.prepareDisplayTaskFor(imageAware, memoryCacheKey);

        listener.onLoadingStarted(uri, imageAware.getWrappedView());
        // 从内存缓存中取出图片
        Bitmap bmp = configuration.memoryCache.get(memoryCacheKey);
        if (bmp != null && !bmp.isRecycled()) {// 图片没被回收
            L.d(LOG_LOAD_IMAGE_FROM_MEMORY_CACHE, memoryCacheKey);
            // 图片是否需要后续处理
            if (options.shouldPostProcess()) {// 需要后续处理
                ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey,
                        options, listener, progressListener, engine.getLockForUri(uri));
                // 新建任务ProcessAndDisplayImageTask
            ...
            } else {// 不需要后续处理,直接显示 BitmapDisplayer display
                options.getDisplayer().display(bmp, imageAware, LoadedFrom.MEMORY_CACHE);
                listener.onLoadingComplete(uri, imageAware.getWrappedView(), bmp);
            }
        } else {// 内存中图片为空，或被回收
            if (options.shouldShowImageOnLoading()) {// 显示加载中图片
            ...
            if (options.isSyncLoading()) {
                ...
                engine.submit(displayTask);// 异步加载：提交到ImageLoaderEngine中
            }
        }
    }
     ...
     *            callback}. Downloaded image will be decoded and scaled to {@link Bitmap} of the size which is <b>equal
     *            or larger</b> (usually a bit larger) than incoming targetImageSize.
     * @param listener
     *            {@linkplain ImageLoadingListener Listener} for image loading process. Listener fires events on UI
     *            thread if this method is called on UI thread.
     * @throws IllegalStateException
     ...
     * {@linkplain ImageLoaderConfiguration.Builder#defaultDisplayImageOptions(DisplayImageOptions) from configuration}
     * will be used.<br />
     * <b>NOTE:</b> {@link #init(ImageLoaderConfiguration)} method must be called before this method call
     * 
     * @param uri
     ...
     * @param options
     *            {@linkplain com.nostra13.universalimageloader.core.DisplayImageOptions Options} for image decoding and
     *            scaling. If <b>null</b> - default display image options
     *            {@linkplain ImageLoaderConfiguration.Builder#defaultDisplayImageOptions(DisplayImageOptions) from
     *            configuration} will be used.
     * @return Result image Bitmap. Can be <b>null</b> if image loading/decoding was failed or cancelled.
     * @throws IllegalStateException
     ...
     *            Image URI (i.e. "http://site.com/image.png", "file:///mnt/sdcard/image.png")
     * @param targetImageSize
     *            Minimal size for {@link Bitmap} which will be returned. Downloaded image will be decoded and scaled to
     *            {@link Bitmap} of the size which is <b>equal or larger</b> (usually a bit larger) than incoming
     *            targetImageSize.
     * @return Result image Bitmap. Can be <b>null</b> if image loading/decoding was failed or cancelled.
     * @throws IllegalStateException
     *             if {@link #init(ImageLoaderConfiguration)} method wasn't called before
     ...
     * 
     * @param imageAware
     *            {@link com.nostra13.universalimageloader.core.imageaware.ImageAware ImageAware} for which display task
     *            will be cancelled
     */
